...
import yemeksepeti_config as config
...
class YemeksepetiApi:
    ...
    def login(self, timeout=5):
        try:
            ...
            api_request = self.SESSION.post(config.ANON_LOGIN, json=login_data, timeout=timeout)
            review_request = self.SESSION.post(config.REVIEW_AUTH)
            self.API_TOKEN = api_request.json()['d']['Result']['Token']['TokenId']
            self.__cfduid = api_request.headers['set-cookie'].split(";")[0].split("=")[1]
    ...
    def search_restaurants(self, catalog="TR_ISTANBUL", area_id="e0b46c42-6718-4c7c-95be-208e835acd6a"):
        """
    ...
    def get_restaurant_reviews(self, category_name="556149dd-3dd5-49d7-b6cf-1f2ebda27bee", catalog="TR_ISTANBUL",
                               area_id="e0b46c42-6718-4c7c-95be-208e835acd6a", page_no=1):
        """
